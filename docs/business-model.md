# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Italy

## Classification

- Repository: `cloud-itonami-iso3166-ita`
- ISO 3166: `ITA` (Italy)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on an Italian
  public contract
...
- registration walkthrough for MEPA (Mercato Elettronico della Pubblica Amministrazione), Italy's national e-procurement marketplace operated by CONSIP
- business/tax registration checklist: a Registro delle Imprese (Business Register) entry via the local Chamber of Commerce, plus a Partita IVA (VAT number); VAT (IVA) registration with the Agenzia delle Entrate
- local-content / preferential-procurement navigation: EU-wide open tendering above EU thresholds (no national-content quota as an EU member state), but Italian-language submission and a domestic/EU address for service are typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Italian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Italy).
