(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Italy-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Italy-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Italy
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(PIVA/統一資格/MEPA登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Italy public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はイタリア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- piva-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-piva? true`, INDEPENDENTLY check
  `:piva-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-piva? e))
                 (not (true? (:piva-verified? e))))
        [{:rule :piva-unverified
          :detail (str subject " はPIVA確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (piva-unverified-violations request st)
