(ns marketentry.store
  "SSoT for the ITA market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems IT" :portal "MEPA"
             ...
             :requires-piva? true :piva-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ITA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "MEPA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-piva? true :piva-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "MEPA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "ITA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "MEPA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "ITA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "MEPA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-piva? true :piva-verified? false
             :drafted? false :submitted? false
             :jurisdiction "ITA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "MEPA"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-piva?)    (assoc :engagement/requires-piva? requires-piva?)
    (some? piva-verified?)    (assoc :engagement/piva-verified? piva-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-piva? :engagement/piva-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-piva? (boolean (:engagement/requires-piva? m))
     :piva-verified? (boolean (:engagement/piva-verified? m))
